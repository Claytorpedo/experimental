...
BENCHMARK_DEFINE_F(EraseFixture, BBXStableErase_EraseInt)(benchmark::State& state) {
	for (auto _ : state) {
		auto vec = vec_;

		auto numErased = bbx::stable_erase(vec, 4);

		benchmark::DoNotOptimize(numErased);
	}
}
BENCHMARK_REGISTER_F(EraseFixture, BBXStableErase_EraseInt)->DO_RANGE();

BENCHMARK_DEFINE_F(EraseFixture, BBXErase_EraseInt)(benchmark::State& state) {
	for (auto _ : state) {
		auto vec = vec_;

		auto numErased = bbx::unstable_erase(vec, 4);

		benchmark::DoNotOptimize(numErased);
	}
}
BENCHMARK_REGISTER_F(EraseFixture, BBXErase_EraseInt)->DO_RANGE();
...
BENCHMARK_DEFINE_F(EraseFixtureString, BBXStableErase_EraseStr)(benchmark::State& state) {
	for (auto _ : state) {
		auto vec = vec_;

		[[maybe_unused]] auto numErased = bbx::stable_erase(vec, EraseFixtureString::Alphabet);

		benchmark::DoNotOptimize(numErased);
	}
}
BENCHMARK_REGISTER_F(EraseFixtureString, BBXStableErase_EraseStr)->DO_RANGE();

BENCHMARK_DEFINE_F(EraseFixtureString, BBXErase_EraseStr)(benchmark::State& state) {
	for (auto _ : state) {
		auto vec = vec_;

		[[maybe_unused]] auto numErased = bbx::unstable_erase(vec, EraseFixtureString::Alphabet);

		benchmark::DoNotOptimize(numErased);
	}
}
BENCHMARK_REGISTER_F(EraseFixtureString, BBXErase_EraseStr)->DO_RANGE();
...
BENCHMARK_DEFINE_F(EraseFixtureLargeClass, BBXStableErase_EraseLargeClass)(benchmark::State& state) {
	for (auto _ : state) {
		auto vec = vec_;

		[[maybe_unused]] auto numErased = bbx::stable_erase(vec, EraseFixtureString::Alphabet, &LargeClass::str);

		benchmark::DoNotOptimize(numErased);
	}
}
BENCHMARK_REGISTER_F(EraseFixtureLargeClass, BBXStableErase_EraseLargeClass)->DO_RANGE();

BENCHMARK_DEFINE_F(EraseFixtureLargeClass, BBXErase_EraseLargeClass)(benchmark::State& state) {
	for (auto _ : state) {
		auto vec = vec_;

		[[maybe_unused]] auto numErased = bbx::unstable_erase(vec, EraseFixtureString::Alphabet, &LargeClass::str);

		benchmark::DoNotOptimize(numErased);
	}
}
BENCHMARK_REGISTER_F(EraseFixtureLargeClass, BBXErase_EraseLargeClass)->DO_RANGE();
